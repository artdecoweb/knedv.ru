...
const PropPage = ({ property, admin, photos }) => {
  ...
  if (property.hidden && !admin) return (<Col>
    <h1 className="CatTitle">Каталог Недвижимости</h1>
    <p>К сожалению, запрашиваемый объект недоступен. Вернитесь в каталог.</p>
  </Col>)
  return (<Col style="padding-bottom:1rem;">
    ...
    {property.hidden && <div className="alert alert-danger mt-3" role="alert">
      Объект выключен и не показывается посетителям. Перейдите к редактированию, чтобы включить его.
    </div>}
...
const Content = ({ offers, categories, property, photos, admin }) => {
  return <div className="container-fluid">
    ...
    <Row>
      <PropPage property={property} admin={admin} photos={photos} />
...
const route = async (ctx) => {
  ...
  if (ctx.params.property) {
    property = await Objects.findOne({ seo: ctx.params.property })
  }
  if (property) {
